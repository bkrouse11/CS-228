...
public class Graph{
  ...
  public void addEdge(int fromIdx, int toIdx, int index, String word) throws IndexOutOfBoundsException, NullPointerException{
	  Edge tmp = new GraphEdge(index, word, vertices[fromIdx], vertices[toIdx]);
	  vertices[fromIdx].edges().add(tmp);
	  
	  Edge tmp2 = new GraphEdge(index, word, vertices[toIdx], vertices[fromIdx]);
	  vertices[toIdx].edges().add(tmp2);
  }
